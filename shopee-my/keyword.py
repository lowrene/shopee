...
def get_prices(driver, k):
    price = 0                                           
    try:                                               
        originalprice = driver.find_element_by_xpath(f'/html/body/div[1]/div/div[3]/div/div[2]/div[3]/div[2]/div[{k}]/a/div/div/div[2]/div[2]/div[1]').text.split(' ')[0].replace('RM', '')
        if ',' in originalprice:                          
            originalprice = float(originalprice.replace(',',''))
    
        else: 
            originalprice = float(driver.find_element_by_xpath(f'/html/body/div[1]/div/div[3]/div/div[2]/div[3]/div[2]/div[{k}]/a/div/div/div[2]/div[2]/div[1]').text.split(' ')[0].replace('RM', ''))

    except: 
        try:                                               
            originalprice = driver.find_element_by_xpath(f'/html/body/div[1]/div/div[3]/div/div[2]/div[3]/div[2]/div[{k}]/a/div/div/div[2]/div[2]/div/span[2]').text.split(' ')[0].replace('RM', '')
            if ',' in originalprice:                          
                originalprice = float(originalprice.replace(',',''))

            else: 
                originalprice = float(driver.find_element_by_xpath(f'/html/body/div[1]/div/div[3]/div/div[2]/div[3]/div[2]/div[{k}]/a/div/div/div[2]/div[2]/div/span[2]').text.split(' ')[0].replace('RM', ''))

        except:
            try:
                originalprice = driver.find_element_by_xpath(f'/html/body/div[1]/div/div[3]/div/div[2]/div[2]/div[2]/div[{k}]/a/div/div/div[2]/div[2]/div[1]').text.split(' ')[0].replace('RM', '')
            except:
                originalprice = 0
...
def get_AllProd(driver, keyword, retrievaldate):
    ...
    for i in range(0, 5): #for each page
        ...
        url = f"https://shopee.com.my/search?keyword={keyword}&page={i}&sortBy=sales"
