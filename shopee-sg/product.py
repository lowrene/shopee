...
def get_prices(driver, k):
    price = 0
    try: 
        originalprice = driver.find_element_by_xpath(f'/html/body/div[1]/div/div[3]/div[1]/div[4]/div[2]/div/div[2]/div[{k}]/a/div/div/div[2]/div[2]/div[1]').text.split(' ')[0].replace('$', '')
        if ',' in originalprice:                          
            originalprice = float(originalprice.replace(',',''))
    
        else: 
            originalprice = float(driver.find_element_by_xpath(f'/html/body/div[1]/div/div[3]/div[1]/div[4]/div[2]/div/div[2]/div[{k}]/a/div/div/div[2]/div[2]/div[1]').text.split(' ')[0].replace('$', ''))

    except: 
        try:
            originalprice = driver.find_element_by_xpath(f'/html/body/div[1]/div/div[3]/div/div[4]/div[2]/div/div[2]/div[{k}]/a/div/div/div[2]/div[2]/div[1]').text.split(' ')[0].replace('$', '')
            if ',' in originalprice:                          
                originalprice = float(originalprice.replace(',',''))

            else: 
                originalprice = float(driver.find_element_by_xpath(f'/html/body/div[1]/div/div[3]/div/div[4]/div[2]/div/div[2]/div[{k}]/a/div/div/div[2]/div[2]/div[1]').text.split(' ')[0].replace('$', ''))
    ...
    try: 
        ...
        try:      
            ...
            try:  
                sleep(0.3)                             
                price = driver.find_element_by_xpath(f'/html/body/div[1]/div/div[3]/div/div[4]/div[2]/div/div[2]/div[{k}]/a/div/div/div[2]/div[2]/div[2]/span[2]').text.split(' ')[0]
    ...
    try:
        ...
        if len(subcat_df)>0:
            ...
            for i in range(len(subcat_df)): #for each category
                logging.info(f"getting category #{i}/{len(subcat_df)}: {subcat_df.iloc[i]['SubCatName']} at {subcat_df.iloc[i]['SubCatURL']}")
